 ...
 *  Amazino Customer Dashboard — Full Auth Integration
 *
 *  Requires: auth.js  (loaded BEFORE this file in the HTML)
 *
 *  Data isolation:
 *    user_data_<userId>   → profile, payments, addresses, notifications
 *    ecommerce_orders     → shared orders (filtered by userId)
 *    ecommerce_cart       → cart items (filtered by userId)
 * ═══════════════════════════════════════════════════════════════
 */
...
const EMAILJS_TEMPLATE = 'template_1o90chi'; // same template as signin OTP
...
let saveCard         = true;
let _verifyOTP       = '';     // OTP generated for personal data verify
let _forgotOTP       = '';     // OTP generated for forgot password flow
...
function loadCurrentUser() {
  ...
  // Legacy fallback
  if (!CU) {
    CU = JSON.parse(localStorage.getItem('ecommerce_current_user') || 'null');
  }

  if (!CU) {
    // AUTH guard should have redirected, but just in case:
    window.location.replace('signin.html');
    return;
  }

  // Normalise name
  if (!CU.name && (CU.firstName || CU.lastName)) {
  ...
  saveUser(); // persist normalised state
  renderUserUI();
  fillPersonalDataForm();
}

/** Persist the current user back to localStorage (keeps AUTH in sync) */
function saveUser() {
  if (!CU) return;
  if (typeof AUTH !== 'undefined' && AUTH.getSession()) {
    AUTH.updateUser(CU);
  } else {
    localStorage.setItem('ecommerce_current_user', JSON.stringify(CU));
  }
}
...
function renderUserUI() {
  ...
  if (CU.profileImage) {
    if (paImg) { paImg.src = CU.profileImage; paImg.style.display = 'block'; }
    if (paIco) paIco.style.display = 'none';
  } else {
    if (paImg) paImg.style.display = 'none';
    if (paIco) paIco.style.display = '';
  }
}
...
function validateNameField(el, errId) {
  ...
  const er=document.getElementById(errId);if(er)er.classList.toggle('show',!ok);
  return ok;
}
...
function validateBirthdayField(el, errId) {
  const d=new Date(el.value);const age=(new Date()-d)/(365.25*24*60*60*1000);
...
function handleImg(inp) {
  ...
  if(!file.type.match(/image\/(jpeg|png|webp|gif)/)){toast('Only JPG/PNG/WEBP/GIF.',true);return;}
  ...
  r.onload=e=>{
    ...
    if(paImg){paImg.src=src;paImg.style.display='block';}if(paIco)paIco.style.display='none';
    const tbA=document.getElementById('tbAvatar');if(tbA)tbA.innerHTML=`<img src="${src}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
    const sbA=document.getElementById('sbAvatar');if(sbA)sbA.innerHTML=`<img src="${src}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
    CU.profileImage=src;saveUser();toast('Profile photo updated!');inp.value='';
  };
  r.onerror=()=>toast('Failed to read image.',true);
...
function selectPayMethod(el, type) {
  ...
  el.classList.add('selected'); selectedPayType=type;
...
function formatExpiry(el) {
  ...
  el.value=v; document.getElementById('cpExpiry').textContent=v||'MM/YY';
}
function toggleSaveCard() {
  saveCard=!saveCard;const el=document.getElementById('saveCardChk');
  el.classList.toggle('unchecked',!saveCard);
  el.innerHTML=saveCard?'<i class="fas fa-check"></i>':'';
}
function confirmPaymentMethod() {
  let ok=true;
  if(selectedPayType==='paypal')ok=validateField('pp_email','pp_email_err',v=>/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(v.trim()));
  else if(selectedPayType==='gpay')ok=validateField('gp_phone','gp_phone_err',v=>v.replace(/\D/g,'').length>=8);
  ...
  if(selectedPayType==='card'){payment.name=document.getElementById('card_name').value.trim();payment.last4=document.getElementById('card_number').value.replace(/\s/g,'').slice(-4);payment.expiry=document.getElementById('card_expiry').value;}
  ...
  ['pp_email','gp_phone','card_name','card_number','card_expiry','card_cvv'].forEach(id=>{const el=document.getElementById(id);if(el){el.value='';el.classList.remove('err','ok');}});
}
function savePaymentDetails() { confirmPaymentMethod(); }
function loadSavedMethods() {
  if(!CU||!CU.payments||!CU.payments.length){document.getElementById('savedMethodsSection').style.display='none';return;}
  document.getElementById('savedMethodsSection').style.display='block';
  const list=document.getElementById('savedMethodsList');list.innerHTML='';
  CU.payments.forEach((p,i)=>{
    ...
    const div=document.createElement('div');div.className='saved-method';
...
function removePayment(i) {
  if(confirm('Remove this payment method?')){CU.payments.splice(i,1);saveUser();loadSavedMethods();toast('Payment method removed.');}
}
...
function loadOrders() {
  ...
  // Filter orders belonging to this user only
  orders = CU ? all.filter(o =>
    String(o.userId) === String(CU.id) ||
    String(o.buyerId) === String(CU.id)
  ).sort((a,b) => new Date(b.createdAt) - new Date(a.createdAt)) : [];

  // Demo orders if still empty
  if (!orders.length) {
    orders = [
      ...
      { id:'ord_demo3', userId:CU?.id, status:'pending',   total:450,  createdAt:new Date().toISOString(),                      items:[{name:'Dining Table',quantity:1},{name:'Chairs',quantity:6}] },
    ];
  }

  setEl('statOrders',  orders.length.toString());
  setEl('statPending', orders.filter(o=>o.status==='pending').length.toString());
  setEl('statSpent',   '$'+orders.reduce((s,o)=>s+o.total,0).toFixed(2));
...
function renderOrders(list, containerId) {
  ...
  el.innerHTML=list.map(o=>`<div class="order-card"><div class="order-top"><div><div class="order-id">#${o.id.slice(-8).toUpperCase()}</div><div class="order-date">${fmtDate(o.createdAt)}</div></div><span class="status-badge ${sm[o.status]||'status-pending'}">${cap(o.status)}</span></div><div class="order-items">${o.items.slice(0,3).map(i=>i.name+' × '+i.quantity).join(' · ')}${o.items.length>3?' · +'+(o.items.length-3)+' more':''}</div><div class="order-footer"><div class="order-total">$${o.total.toFixed(2)}</div><button class="btn-view" onclick="viewOrder('${o.id}')"><i class="fas fa-eye" style="margin-right:4px;font-size:.7rem;"></i>View</button></div></div>`).join('');
}

function renderRecentOrders(list) {
  const el=document.getElementById('recentOrdersList');if(!el||!list.length)return;
  const sm={pending:'status-pending',processing:'status-processing',shipped:'status-shipped',delivered:'status-delivered'};
  el.innerHTML=`<div class="table-wrap"><table class="data-table"><thead><tr><th>Order ID</th><th>Date</th><th>Items</th><th>Total</th><th>Status</th><th></th></tr></thead><tbody>${list.map(o=>`<tr><td style="font-weight:800;">#${o.id.slice(-8).toUpperCase()}</td><td style="color:var(--mu);">${fmtDate(o.createdAt)}</td><td style="color:var(--mu);">${o.items.length} item(s)</td><td style="font-weight:700;">$${o.total.toFixed(2)}</td><td><span class="status-badge ${sm[o.status]||'status-pending'}">${cap(o.status)}</span></td><td><button class="btn-view" onclick="viewOrder('${o.id}')">View</button></td></tr>`).join('')}</tbody></table></div>`;
}
...
function viewOrder(id) {
  ...
  const sm={pending:'status-pending',processing:'status-processing',shipped:'status-shipped',delivered:'status-delivered'};
  document.getElementById('orderModalBody').innerHTML=`<div class="order-info-row"><span class="order-info-label">Order ID</span><span class="order-info-val" style="font-family:monospace;">#${o.id.slice(-8).toUpperCase()}</span></div><div class="order-info-row"><span class="order-info-label">Date Placed</span><span class="order-info-val">${fmtDate(o.createdAt)}</span></div><div class="order-info-row"><span class="order-info-label">Status</span><span class="status-badge ${sm[o.status]||'status-pending'}">${cap(o.status)}</span></div><div class="order-info-row"><span class="order-info-label">Payment</span><span class="order-info-val">Credit Card</span></div><div class="order-items-title">Items in this order</div>${o.items.map(item=>`<div class="order-item-row"><div class="order-item-dot"></div><span class="order-item-name">${item.name}</span><span class="order-item-qty">× ${item.quantity}</span></div>`).join('')}<div class="order-total-row"><span>Order Total</span><span style="color:var(--pri);">$${o.total.toFixed(2)}</span></div>`;
...
function loadSavedAddresses() {
  if(!CU){if(CU)CU.addresses=[];return;}
  const el=document.getElementById('savedAddressesList');el.innerHTML='';
  (CU.addresses||[]).forEach((a,i)=>{
    const div=document.createElement('div');div.className='address-card';
    div.innerHTML=`<div><div class="addr-name">${a.name}</div><div class="addr-text">${a.street}</div></div><div class="addr-actions"><button class="btn-edit" onclick="editAddress(${i})">Edit</button><button class="btn-del" onclick="deleteAddress(${i})">Delete</button></div>`;
    el.appendChild(div);
  });
}
...
function editAddress(i) {
  const a=CU.addresses[i];const parts=a.name.split(' ');
  setVal('addr_fname',parts[0]||'');setVal('addr_lname',parts.slice(1).join(' ')||'');
  setVal('addr_company',a.company||'');setVal('addr_street',a.street||'');
  setVal('addr_city',a.city||'');setVal('addr_state',a.state||'');
  setVal('addr_country',a.country||'');setVal('addr_zip',a.zip||'');
  CU.addresses.splice(i,1);saveUser();loadSavedAddresses();
...
function deleteAddress(i) {
  if(confirm('Delete this address?')){CU.addresses.splice(i,1);saveUser();loadSavedAddresses();toast('Address removed.');}
}
...
function checkPasswordStrength(el) {
  const v=el.value;const bar=document.getElementById('strengthBar');let score=0;
  if(v.length>=8)score++;if(/[A-Z]/.test(v))score++;if(/[0-9]/.test(v))score++;if(/[^A-Za-z0-9]/.test(v))score++;
  bar.style.width=(score/4*100)+'%';bar.style.background=['#e74c3c','#e67e22','#f1c40f','#2ecc71'][score-1]||'#e74c3c';
}
...
async function submitPassword(e) {
  ...
  try {
    // Verify old password via API, then PATCH new password
    ...
    if(err.message&&err.message.includes('incorrect')){
      document.getElementById('pw_current').classList.add('err');
      document.getElementById('pw_current_err').classList.add('show');
...
function loadNotifications() {
  ...
  const notifs=CU?.notifications?.length?CU.notifications:[
    ...
    {icon:'fa-check-circle',text:'Payment completed for your recent order. Total: $89.99',time:'1 day ago'},
...
/* ════════════════════════════════════════════════════════════
   VERIFY MODAL (personal data)
════════════════════════════════════════════════════════════ */
/* ════════════════════════════════════════════════════════════
   VERIFY MODAL (personal data) — real OTP via EmailJS
════════════════════════════════════════════════════════════ */
function openModal() {
  _verifyOTP = Math.floor(1000 + Math.random() * 9000).toString();
  const modal = document.getElementById('verifyModal');
  modal.classList.add('open');

  // Show sending state
  const hint = document.querySelector('.code-hint');
  if (hint) hint.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Sending code to your email…';

  // Disable verify button while sending
  ...
  emailjs.send(EMAILJS_SERVICE, EMAILJS_TEMPLATE, {
    ...
    if (hint) hint.innerHTML = `Code sent to <strong style="color:var(--pri);">${CU.email}</strong>`;
    if (verifyBtn) verifyBtn.disabled = false;
    document.getElementById('code1').focus();
  }, () => {
    // EmailJS failed — fallback to demo code
    _verifyOTP = '1234';
    if (hint) hint.innerHTML = `Couldn't send email. Use demo code <strong>1234</strong>`;
    if (verifyBtn) verifyBtn.disabled = false;
    document.getElementById('code1').focus();
    toast('Email service unavailable. Using demo code 1234.', true);
  });
}
function closeModal() {
...
function doVerify() {
  ...
  if (CU.id) {
    const API = 'https://699c4912110b5b738cc24139.mockapi.io/api/ecomerce/users/users_table';
    const payload = {
      name:     CU.name,
      email:    CU.email,
      phone:    CU.phone,
    ...
    .then(r => r.ok ? toast('Profile updated successfully!') : toast('Profile saved locally. API sync failed.', true))
    .catch(() => toast('Profile saved locally. No internet connection.', true));
  } else {
...
function setupCodeBoxes() {
  ...
  wireBoxes(['code1','code2','code3','code4'], doVerify);
  wireBoxes(['fpCode1','fpCode2','fpCode3','fpCode4'], verifyForgotOtp);
}

/* ════════════════════════════════════════════════════════════
   FORGOT PASSWORD — IN-DASHBOARD FLOW
════════════════════════════════════════════════════════════ */
const FP_API = 'https://699c4912110b5b738cc24139.mockapi.io/api/ecomerce/users/users_table';

function openForgotPasswordFlow(e) {
  if(e) e.preventDefault();
  // Pre-fill user email and show modal
  const emailEl = document.getElementById('forgotPwEmail');
  const emailConfirmEl = document.getElementById('forgotPwEmailConfirm');
  if(emailEl) emailEl.textContent = CU?.email || '';
  if(emailConfirmEl) emailConfirmEl.textContent = CU?.email || '';
  _showForgotStep(1);
  document.getElementById('forgotPwModal').classList.add('open');
}

function closeForgotPwModal() {
  document.getElementById('forgotPwModal').classList.remove('open');
  _forgotOTP = '';
  // Clear all inputs
  ['fpCode1','fpCode2','fpCode3','fpCode4'].forEach(id=>{
    const el=document.getElementById(id); if(el){el.value='';el.classList.remove('err');}
  });
  ['fp_new_pw','fp_confirm_pw'].forEach(id=>{
    const el=document.getElementById(id); if(el){el.value='';el.classList.remove('err','ok');}
  });
  const bar=document.getElementById('fpStrengthBar'); if(bar) bar.style.width='0';
}

function _showForgotStep(n) {
  [1,2,3].forEach(i => {
...
async function sendForgotOtp(isResend=false) {
  if(!CU?.email){ toast('No email on account.',true); return; }
  const btn = document.getElementById('forgotSendBtn');
  const origHtml = btn ? btn.innerHTML : '';
  if(btn){ btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Sending…'; btn.disabled=true; }

  _forgotOTP = Math.floor(1000 + Math.random() * 9000).toString();
...
function verifyForgotOtp() {
  const code=['fpCode1','fpCode2','fpCode3','fpCode4'].map(id=>{
    const e=document.getElementById(id); return e?e.value:'';
  }).join('');
  if(code !== _forgotOTP){
    ['fpCode1','fpCode2','fpCode3','fpCode4'].forEach(id=>{
  ...
  _showForgotStep(3);
  document.getElementById('fp_new_pw').focus();
}

function checkFpStrength(el) {
  const v=el.value; const bar=document.getElementById('fpStrengthBar'); let score=0;
  if(v.length>=8)score++; if(/[A-Z]/.test(v))score++; if(/[0-9]/.test(v))score++; if(/[^A-Za-z0-9]/.test(v))score++;
...
async function doForgotPasswordReset() {
  ...
  const confPw = document.getElementById('fp_confirm_pw').value;
  let ok = true;

  // Validate
  const pwEl  = document.getElementById('fp_new_pw');
  const pwErr = document.getElementById('fp_new_pw_err');
  if(newPw.length < 8){
    pwEl.classList.add('err'); if(pwErr) pwErr.classList.add('show'); ok=false;
  } else {
    pwEl.classList.remove('err'); pwEl.classList.add('ok'); if(pwErr) pwErr.classList.remove('show');
  }

  const confEl  = document.getElementById('fp_confirm_pw');
  const confErr = document.getElementById('fp_confirm_pw_err');
  if(newPw !== confPw || !confPw){
    confEl.classList.add('err'); if(confErr) confErr.classList.add('show'); ok=false;
  } else {
    confEl.classList.remove('err'); confEl.classList.add('ok'); if(confErr) confErr.classList.remove('show');
  }
  if(!ok){ toast('Please fix the errors.',true); return; }

  // Reset via AUTH.changePassword if possible, else direct API call
  const resetBtn = document.querySelector('#forgotStep3 .btn-primary');
  ...
  try {
    // Direct API PATCH (no old password needed — user verified via OTP)
    const res = await fetch(`${FP_API}/${CU.id}`, {
      method:  'PUT',
      headers: {'Content-Type':'application/json'},
    ...
    closeForgotPwModal();
    // Clear the password form fields too
    ['pw_current','pw_new','pw_confirm'].forEach(id=>{
      const el=document.getElementById(id); if(el){el.value='';el.classList.remove('err','ok');}
    });
    document.getElementById('strengthBar').style.width='0';
...
function logout() {
  ...
  const av=document.getElementById('logoutAvatar');
  if(CU?.profileImage){av.innerHTML=`<img src="${CU.profileImage}" alt="" style="width:100%;height:100%;object-fit:cover;display:block;">`;}
  else{av.textContent=initials;}
  setEl('logoutName', CU?.name||'User');
...
function updateCartCount() {
  const c=JSON.parse(localStorage.getItem(K.CART)||'[]');
  const el=document.getElementById('cartCount');
  if(el)el.textContent=c.filter(i=>!i.userId||String(i.userId)===String(CU?.id)).length;
}
